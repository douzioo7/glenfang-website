...
- `/guestbook/`：独立留言板，复用现有 Twikoo 服务，固定评论路径为 `/guestbook/`。文章评论仍按各自路径隔离。审核、反垃圾和邮件通知沿用 Twikoo 后台配置。
- 背景使用本地纸纹和按栏目切换的静态色晕；首页装饰仅在鼠标设备且未启用“减少动态效果”时轻微跟随，无持续动画循环。触屏和文章页面保持静态。
